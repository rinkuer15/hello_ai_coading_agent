# Devox Configuration Guide

Interactive guide for viewing and modifying Devox configuration. Use this when the user wants to change, view, or understand their config — at any point, not just during initial setup.
...
Options:
  1. "Repo config" (Recommended) — Settings for a specific repository (.devox/config.yaml in the repo)
  2. "Global config" — User-wide settings (~/.devox/config.yaml)
  3. "Both" — Review and modify both configs
```

## Step 2: Load Current Config

### For Global Config (~/.devox/config.yaml)

Read the file:
```bash
cat ~/.devox/config.yaml
```

If it doesn't exist, tell the user it will be auto-created on first Devox run with defaults. Offer to create it now.

### For Repo Config (<repo>/.devox/config.yaml)

First determine the target repo. If the current working directory is a git repo (and not the Devox source repo), use it. Otherwise ask:
...
Then read the file:
```bash
cat <target-repo>/.devox/config.yaml
```

If it doesn't exist, tell the user: "No repo config found. I can create one — or Devox will use defaults." Offer to create it.
...
Format example:
```
Current repo config (.devox/config.yaml):
...
Options:
  1. "Bot name" — Display name shown in messages (current: {value}, default: Devox)
...
After writing, confirm: "Updated {path}. Changes take effect on the next Devox invocation (no restart needed for CLI)."
...
### Global Config (~/.devox/config.yaml)

| Option | Type | Default | Description |
|--------|------|---------|-------------|
| `botName` | string | `Devox` | Bot display name shown in messages |
...
| `paths.workspaces` | string | `~/.devox/workspaces` | Directory for cloned repositories |
| `paths.worktrees` | string | `~/.devox/worktrees` | Directory for git worktrees |
| `concurrency.maxConversations` | number | `10` | Maximum concurrent AI conversations |

### Repo Config (<repo>/.devox/config.yaml)
...
2. **Repo config** (`.devox/config.yaml` in repo) — project-specific
3. **Global config** (`~/.devox/config.yaml`) — user-wide preferences
...
| `DEVOX_HOME` | Base directory for all Devox paths |
