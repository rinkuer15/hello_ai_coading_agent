...
The user is inside the **remote-coding-agent** repository — that's how they have access to this skill. The Devox repo path is the current working directory. Store it as `<devox-repo>`.

## Step 1: Ask for Target Repo

**IMPORTANT**: The target repo is the user's own project — **never** the remote-coding-agent (Devox) repo itself. Do not suggest or offer the current directory as an option.

Use **AskUserQuestion** with a single question:

```
Header: "Target repo"
Question: "What is the path to the repository you want to work on using Devox? (This should be your own project, not the Devox repo.)"
Options:
  1. "Clone from GitHub" — user provides a GitHub URL; clone it to ~/.devox/workspaces/
```
...
```bash
devox-repo-path=$(pwd)
mkdir -p ~/.devox/workspaces
cd ~/.devox/workspaces && git clone <url>
```
...
1. `cd <devox-repo> && ~/.bun/bin/bun install` (or `bun install` if bun was already in PATH)
2. `cd <devox-repo>/packages/cli && ~/.bun/bin/bun link` (or `bun link`)
3. Verify: `devox version`
4. Check Claude is installed: `which claude`, then `claude /login` if needed

> **Note — Claude Code binary path.** Devox does not bundle Claude Code. In compiled Devox binaries (quick install, Homebrew), the Claude Code SDK needs `CLAUDE_BIN_PATH` set to the absolute path of its `cli.js`. The `devox setup` wizard in Step 4 auto-detects this via `npm root -g` and writes it to `~/.devox/.env` — no manual action needed in the typical case. Source installs (`bun run`) don't need this; the SDK finds `cli.js` via `node_modules` automatically.

## Step 4: Configure Credentials

The CLI loads infrastructure config (database, tokens) from `~/.devox/.env` only. This prevents conflicts with project `.env` files that may contain different database URLs.
...
**CRITICAL**: Do NOT run `devox setup` directly via Bash — it requires interactive input that I cannot provide. The `--spawn` flag attempts to open a new terminal window where the user can interact directly.
...
>
> It saves configuration to both `~/.devox/.env` and the repo `.env`."
...
| Issue | Cause | Fix |
|-------|-------|-----|
| Shows `sqlite` but expected `postgresql` | `~/.devox/.env` missing or no DATABASE_URL | Run `devox setup` again in your terminal |
...
Bundled default commands and workflows are loaded automatically at runtime from the Devox installation — nothing needs to be copied into the target repo.

Tell the user:
- "Default commands and workflows are loaded automatically at runtime — no files are added to your repo."
- "To browse defaults, look in `<devox-repo>/.devox/commands/defaults/` and `<devox-repo>/.devox/workflows/defaults/`."
- "To customize a default, copy the specific file into your repo's `.devox/commands/` or `.devox/workflows/` directory with the same filename. Your repo version takes priority over the bundled default."

## Step 7: Start the Server (non-CLI platforms only)

**Skip if "CLI only" was selected.**

After all platform tokens are in `.env`, read and follow `guides/server.md` to start the server from the devox repo.
...
```bash
cd <target-repo> && devox workflow list
```

If the CLI is working, also run:

```bash
cd <target-repo> && devox workflow run devox-assist "Say hello"
```
...
| Error | Cause | Fix |
|-------|-------|-----|
| `devox: command not found` | CLI not linked | Re-run `cd <devox-repo>/packages/cli && bun link` |
| `Not a git repository` | Not in a git repo | `cd` to the target repo root |
| `No workflows found` | Missing `.devox/workflows/` | Default workflows load automatically — check `devox version` works first |
| Auth errors | Claude not authenticated | Run `claude /login` |
| `relation "remote_agent_*" does not exist` | DATABASE_URL missing or tables not created | Ensure `~/.devox/.env` has DATABASE_URL and run migrations |
| `Database: sqlite` but expected PostgreSQL | `~/.devox/.env` missing DATABASE_URL | Add DATABASE_URL to `~/.devox/.env` |
...
```
Header: "Devox skill"
Question: "Would you like to copy the Devox skill into your target repo so Coding Agents can invoke Devox workflows from there?"
Options:
  1. "Yes, copy the skill" — Copies .claude/skills/devox/ into the target repo. This will appear in git unless you gitignore it.
  2. "No, skip" — You can always run Devox workflows from the Devox repo instead.
```
...
cp -r <devox-repo>/.claude/skills/devox <target-repo>/.claude/skills/devox
```
...
2. `cd <target-repo>`
3. Run `claude` to launch Claude Code or run `copilot` to launch Copilot Code or similarly other supported coding agents.
4. The devox skill is now loaded — ask Agent to run workflows, fix issues, review PRs, etc.

Example first command in the target repo:
```
"Use devox to fix issue #1"
```

Tell the user:
- Default commands and workflows load automatically at runtime — nothing was added to your repo
- `.claude/skills/devox/` — skill for Claude Code integration
- To customize a default, copy the specific file from `<devox-repo>/.devox/commands/defaults/` or `<devox-repo>/.devox/workflows/defaults/` into your repo's `.devox/commands/` or `.devox/workflows/` with the same filename

**Important**: End the summary with this message:

> "If you want to configure advanced options later — like changing the default AI assistant, customizing worktree behavior, or adjusting which files get copied into isolated environments — just ask me to help you with 'devox config' and I'll walk you through it."

The end state: user is in their target repo with the Devox skill available, defaults loaded at runtime without polluting the repo, using Claude Code as the interface.
...
|----------|---------|---------|
| `~/.devox/.env` | **CLI** | Global infrastructure config — database, AI tokens |
| `<devox-repo>/.env` | **Server** | Platform tokens for Telegram/Slack/GitHub/Discord |

**Best practice**: Use `~/.devox/.env` as the single source of truth. Symlink or copy to `<devox-repo>/.env` if running the server.

**Note**: The CLI does NOT load `.env` from the current working directory. This prevents conflicts when running Devox from projects that have their own database configurations.
...
|----------|-------|---------|
| `~/.devox/config.yaml` | Global | Default AI assistant, streaming modes, concurrency |
| `<repo>/.devox/config.yaml` | Per-repo | AI assistant, worktree settings, commands config |
...
| `commands.folder` | string | `.devox/commands` | Custom command folder path (relative to repo root) |
