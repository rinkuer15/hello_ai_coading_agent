...
- `devox-piv-loop` — Plan-Implement-Validate with iterative feedback
- `devox-interactive-prd` — Guided PRD creation with approval gates
...
```bash
devox workflow run <name> "<message>"
```

### 2. Monitor for pause

Check `devox workflow status` periodically. When status changes to `paused`, the workflow is waiting for user input.
...
```bash
# Find the log file
find ~/.devox/workspaces -name "<run-id>.jsonl" 2>/dev/null
...
```bash
devox workflow approve <run-id> "<user's exact response>"
```
...
devox workflow approve <run-id> "your feedback or answers here"

# Reject (cancels the workflow)
devox workflow reject <run-id> "reason for rejection"
```

## Troubleshooting

- **Workflow shows `running` for a long time**: The AI is doing research/implementation. Be patient — check again in a few minutes.
- **Log file not found**: The log is at `~/.devox/workspaces/<owner>/<repo>/logs/<run-id>.jsonl`
- **User wants to cancel**: Run `devox workflow reject <run-id>` or `devox workflow cancel <run-id>`
