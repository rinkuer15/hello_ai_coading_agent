...
One-page lookup for Devox workflow parameters: which field works on which node type, how to pick the right parameter for a given intent, and the gotchas that don't fail loudly.
...
| Give a node external tools                       | `mcp: .devox/mcp/server.json`                               |
...
See [devox.diy/guides/authoring-workflows/#inline-sub-agents](https://devox.diy/guides/authoring-workflows/#inline-sub-agents) for a worked end-to-end example.
...
| Repo initialization, `.devox/config.yaml` schema | `repo-init.md`                                                         |
...
| Full schema reference                            | [devox.diy/reference/configuration/](https://devox.diy/reference/configuration/) |
...
2. Validate before running: `devox validate workflows <name>`.
