...
└── defaults/               # Optional: override bundled defaults
    └── devox-assist.md    # Overrides the bundled devox-assist
```
...
When a workflow references `command: my-command`, Devox searches in this order:
1. `.devox/commands/my-command.md` (repo custom)
2. `.devox/commands/defaults/my-command.md` (repo default overrides)
3. Bundled defaults (shipped with Devox)
